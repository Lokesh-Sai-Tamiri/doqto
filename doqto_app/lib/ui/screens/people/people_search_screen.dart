...
/// People search (M3). Debounced query → latest-wins notifier. Row tap opens
/// the addressable profile; the trailing button does a lightweight inline
/// optimistic invite (see [_InlineConnect]) so users can connect without a
/// round-trip through the profile screen.
...
class _Body extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return switch (state) {
      PeopleSearchIdle() => const _Message(
          icon: Icons.search,
          text: Strings.netSearchPrompt,
        ),
...
class _ResultsList extends StatelessWidget {
  ...
  const _ResultsList({required this.people});
